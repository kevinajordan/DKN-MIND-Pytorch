...
class KCNN(torch.nn.Module):
    ...
    def __init__(self, window_sizes, num_word_tokens, word_dim, kb_dim, device,
                entity_embedding, context_embedding):
        super(KCNN, self).__init__()
        self.config = config
        self.word_embedding = nn.Embedding(num_word_tokens, word_dim)
        self.entity_embedding = entity_embedding
        self.context_embedding = context_embedding
        self.transform_matrix = nn.Parameter(torch.empty(word_dim, kb_dim))
        self.transform_bias = nn.Parameter(torch.empty(word_dim))
